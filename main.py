...
def random_cof():
    if request.method == "GET":
        ...
        all_caffe = result.scalars().all()
        random_caffe = random.choice(all_caffe)
        response = jsonify(cafe=random_caffe.to_dict())
...
def all_cof():
    ...
    all_caffe = results.scalars().all()

    return jsonify(cafes=[cafe.to_dict() for cafe in all_caffe])
